...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([mygraph], width=9),dbc.Col([dropdown], width=3)
    ]),
    # dbc.Row([
...
def update_graph(column_name):  # function arguments come from the component property of the Input
    ...
    fig = px.bar(data_frame=df,x='STATE', y=f'{column_name}',height=600,animation_frame='YEAR')

    fig = px.line(data_frame=df,x='YEAR',y=f'{column_name}',color='STATE')
